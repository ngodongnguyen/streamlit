...
input_numbers = st.text_area("Nhập danh sách số tiền:", height=200, value="")

if st.button("Chuyển đổi"):
    if input_numbers:
        output_numbers = convert_and_divide(input_numbers)
        st.subheader("Kết quả đã chuyển đổi:")
        output_area = st.text_area("Danh sách số với dấu phẩy:", value=output_numbers, height=200)
        
        # Nút copy sử dụng HTML và JavaScript
        st.markdown("""
        <button onclick="copyToClipboard()">Copy kết quả</button>
        <script>
        function copyToClipboard() {
            var textarea = document.querySelectorAll('textarea')[1];  // Chọn textarea thứ hai (kết quả)
            textarea.select();
